...
class FluxSingleTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # FluxRegionalAttnProcessor2_0 always returns a tuple.
        ...
        if is_use_cond:
            if self.conditional_integration_method == "unified":
                ...
                # Joint-attention using shared projections from self.attn
                cond_attn_output = cond_joint_attention(
                    attn=self.attn,
                    hidden_states=norm_hidden_states,
                ...
                attn_output2, cond_attn_output = cond_attn_output[:2]
...
class FluxTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if is_use_cond:
            if self.conditional_integration_method == "unified":
                cond_attn_output = result[2]
            else:
                # image_rotary_emb covers [txt, img]; slice to image-only portion for the query
...
class FluxTransformer2DModel(
    ...
    def forward(
        ...
        if is_use_adaptive_mask and is_use_hard_mask:
            ...
            # When cond2image_attention_weight == 0 the user wants the cond
            # branch fully off.
            if cond2image_attention_weight <= 0 and cond_hidden_states is not None:
                total_txt = encoder_hidden_states.shape[1]
                img_seq = hidden_states.shape[1]
                cond_total = cond_hidden_states.shape[1]
                img_start = total_txt
                img_end = img_start + img_seq
                cond_start = img_end
                cond_end = cond_start + cond_total
                adaptive_blocked[:, img_start:img_end, cond_start:cond_end] = False
                adaptive_blocked[:, cond_start:cond_end, img_start:img_end] = False
                adaptive_soft_bias[:, img_start:img_end, cond_start:cond_end] = float('-inf')
                adaptive_soft_bias[:, cond_start:cond_end, img_start:img_end] = float('-inf')
