...
class AdaptiveMaskSpec:
    """Lazy adaptive attention mask, consumed by FlexAttention or SDPA-math.
    ...
    Attributes:
        ...
        cross_control: optional (B, N, N) bool. True at cells that must stay
                       HARD -inf (inter-stream isolation: seg-omini,
                       omini_i-omini_j). Overrides the AMM softening at those
                       positions.
    """
    blocked: torch.Tensor
    strengths: torch.Tensor
    soft_bias: Optional[torch.Tensor] = None
    cross_control: Optional[torch.Tensor] = None
...
def _flex_aligned(query, key, value, spec: AdaptiveMaskSpec):
    ...
    cross_control = spec.cross_control
    ...
    if not _USE_FLEX:
        ...
        if cross_control is not None:
            # Cross-stream cells stay hard -inf, regardless of AMM softening.
            bias = torch.where(
                cross_control[:, None, :, :],
                torch.full((), float("-inf"), dtype=bias.dtype, device=bias.device),
                bias,
            )
        # Force math backend
    ...
    if soft_bias is None and cross_control is None:
        ...
        def score_mod(score, b, h, q_idx, kv_idx):
            ...
            return torch.where(
                cross_control[b, q_idx, kv_idx],
                torch.full_like(score, float("-inf")),
                base,
            )
    elif cross_control is None:
        def score_mod(score, b, h, q_idx, kv_idx):
            is_blocked = blocked[b, q_idx, kv_idx]
            return torch.where(
                is_blocked,
...
def _sdpa_aligned(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False):
    ...
    AdaptiveMaskSpec inputs route to FlexAttention / SDPA-math via _flex_aligned.
    Tensor masks with requires_grad force the SDPA math backend.
    Plain tensor masks pass through to standard SDPA (flash if mask is None).
    """
    if isinstance(attn_mask, AdaptiveMaskSpec):
        if not _FLEX_AVAILABLE and _USE_FLEX:
            raise RuntimeError(
                "AdaptiveMaskSpec requires FlexAttention (torch >= 2.5) when "
                f"USE_FLEX=1. Got torch={torch.__version__}. Set USE_FLEX=0 "
                "to use the SDPA-math fallback instead."
            )
...
class FluxRegionalAttnProcessor2_0:
    ...
    def __call__(
            ...
            omini_cond_hidden_states_list: Optional[List[torch.FloatTensor]] = None,
            omini_cond_rotary_embs: Optional[List] = None,
        ...
        # Save input seq lengths for output splitting
        img_seq_len = hidden_states.shape[1]
        enc_seq_len = encoder_hidden_states.shape[1] if encoder_hidden_states is not None else 0
        cond_seq_len = cond_hidden_states.shape[1] if cond_hidden_states is not None else 0
        omini_seq_lens = [h.shape[1] for h in omini_cond_hidden_states_list] if is_use_omini else []
        ...
        # Project Q/K/V for each omini condition with its own LoRA adapter
        if is_use_omini:
            for cond_i, omini_hs in enumerate(omini_cond_hidden_states_list):
                with select_lora((attn.to_q, attn.to_k, attn.to_v), omini_adapter_names[cond_i]):
                    omini_query = attn.to_q(omini_hs)
                    omini_key = attn.to_k(omini_hs)
                    omini_value = attn.to_v(omini_hs)

                omini_query = omini_query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
                omini_key = omini_key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
                omini_value = omini_value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)

                if attn.norm_q is not None:
                    omini_query = attn.norm_q(omini_query)
                if attn.norm_k is not None:
                    omini_key = attn.norm_k(omini_key)
                if omini_cond_rotary_embs is not None and omini_cond_rotary_embs[cond_i] is not None:
                    omini_query = apply_rotary_emb(omini_query, omini_cond_rotary_embs[cond_i])
                    omini_key = apply_rotary_emb(omini_key, omini_cond_rotary_embs[cond_i])
                query = torch.cat([query, omini_query], dim=2)
                key = torch.cat([key, omini_key], dim=2)
                value = torch.cat([value, omini_value], dim=2)
        ...
        # Split output by saved sequence lengths.
        # Order: [enc, img, cond, omini_0, omini_1, ...]
        offset = 0
        enc_out = None
        if encoder_hidden_states is not None:
            enc_out = hidden_states[:, offset:offset + enc_seq_len]
            offset += enc_seq_len
        img_out = hidden_states[:, offset:offset + img_seq_len]
        offset += img_seq_len
        cond_out = None
        if cond_seq_len > 0:
            cond_out = hidden_states[:, offset:offset + cond_seq_len]
            offset += cond_seq_len
        omini_outs = []
        for omini_len in omini_seq_lens:
            omini_outs.append(hidden_states[:, offset:offset + omini_len])
            offset += omini_len

        # Apply output projections (double blocks only; single blocks have pre_only=True)
        if encoder_hidden_states is not None:
            if cond_out is not None:
                with select_lora((attn.to_out[0],), 'cond'):
                    cond_out = attn.to_out[0](cond_out)
                    cond_out = attn.to_out[1](cond_out)
            for cond_i in range(len(omini_outs)):
                with select_lora((attn.to_out[0],), omini_adapter_names[cond_i]):
                    omini_outs[cond_i] = attn.to_out[0](omini_outs[cond_i])
                    omini_outs[cond_i] = attn.to_out[1](omini_outs[cond_i])
        ...
        # Build return tuple.
        # Double blocks (enc present): (img, enc[, cond][, omini_0, omini_1, ...])
        # Single blocks (no enc): (img[, cond][, omini_0, omini_1, ...])
        if enc_out is not None:
            results = [img_out, enc_out]
        else:
            results = [img_out]
        if cond_out is not None:
            results.append(cond_out)
        results.extend(omini_outs)
