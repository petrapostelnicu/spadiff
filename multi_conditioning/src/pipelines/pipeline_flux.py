...
from multi_conditioning.src.models.transformer_flux import FluxTransformer2DModel
...
class FluxRegionalPipeline(FluxPipeline):
    ...
    def prepare_attention_mask(
            ...
            omini_cond_seq_lens: Optional[list[list[int]]] = None,
            omini_cond_weights: Optional[list[float]] = None,
    ):
        """
        Prepare attention masks for regional control with optional multi-conditioning.
        ...
        Args:
            ...
            cond2image_attention_weight: attention weight between seg condition and image tokens
            omini_cond_seq_lens: list[list[int]] - per-batch list of omini condition seq lengths
                                 e.g. [[256, 256], [256, 256]] for 2 omini conditions in batch of 2
            omini_cond_weights: list[float] - per-condition attention weight for each omini condition
                                e.g. [1.0, 0.5] means first omini cond at full strength, second at half
        """
        ...
        batch_size = len(regional_labels)

        # Total omini seq len (must be same across batch since they're concatenated)
        total_omini_seq_len = sum(omini_cond_seq_lens[0]) if omini_cond_seq_lens else 0
        ...
        for bs_i, label in enumerate(regional_labels):
            ...
            # sequence: global_txt_token, regional_txt_token, padding_txt_token, noisy_image_token, [condition_token, padding_token], [omini_0, omini_1, ...]
            total_seq_len = total_txt_seq_len + img_seq_len + cond_seq_len + pad_seq_len + total_omini_seq_len
            ...
            # If we have seg condition tokens, set up img+cond attention
            if cond_seq_len > 0:
                cond_start = img_end
                cond_end = cond_start + cond_seq_len
                # img+cond attends to itself
                attention_mask[img_start:cond_end, img_start:cond_end] = True

                # cond to image with weight
                attention_mask[img_start:img_end, cond_start:cond_end] = cond2image_attention_weight
                attention_mask[cond_start:cond_end, img_start:img_end] = cond2image_attention_weight

            # If we have OminiControl condition tokens, set up omini-img attention
            if total_omini_seq_len > 0:
                omini_start = img_end + cond_seq_len + pad_seq_len
                omini_lens = omini_cond_seq_lens[bs_i]
                # Default to 1.0 for each condition if no weights provided
                weights = omini_cond_weights if omini_cond_weights is not None else [1.0] * len(omini_lens)
                offset = omini_start
                for cond_i, omini_len in enumerate(omini_lens):
                    omini_end = offset + omini_len
                    w = weights[cond_i]
                    # omini to image with per-condition weight
                    attention_mask[offset:omini_end, img_start:img_end] = w
                    # image to omini with per-condition weight
                    attention_mask[img_start:img_end, offset:omini_end] = w
                    # each omini group self-attention
                    attention_mask[offset:omini_end, offset:omini_end] = True
                    offset = omini_end
            ...
            if pad_seq_len != 0:
                ...
                pad_start = img_end + cond_seq_len
                pad_end = pad_start + pad_seq_len
                attention_mask[:, pad_start:pad_end] = False
                attention_mask[pad_start:pad_end, :] = False
                attention_mask[pad_start:pad_end, pad_start:pad_end] = True
            ...
            # Build cross-control mask
            seg_off = cond_seq_len > 0 and cond2image_attention_weight <= 0.0
            omini_lens_bs = omini_cond_seq_lens[bs_i] if omini_cond_seq_lens else []
            omini_weights_bs = omini_cond_weights if omini_cond_weights is not None \
                else [1.0] * len(omini_lens_bs)
            any_omini_off = any(w <= 0.0 for w in omini_weights_bs)
            if (total_omini_seq_len > 0
                    or cond_seq_len > 0
                    or seg_off
                    or any_omini_off):
                cc_mask = torch.zeros((total_seq_len, total_seq_len), dtype=dtype, device=device)
                omini_start = img_end + cond_seq_len + pad_seq_len
                cond_start = img_end
                cond_end = cond_start + cond_seq_len

                # Block seg_cond - each omini group
                if cond_seq_len > 0 and total_omini_seq_len > 0:
                    cc_mask[cond_start:cond_end, omini_start:omini_start + total_omini_seq_len] = float('-inf')
                    cc_mask[omini_start:omini_start + total_omini_seq_len, cond_start:cond_end] = float('-inf')

                # When seg cond2image_attention_weight == 0, force img↔seg_cond hard -inf.
                if seg_off:
                    cc_mask[img_start:img_end, cond_start:cond_end] = float('-inf')
                    cc_mask[cond_start:cond_end, img_start:img_end] = float('-inf')

                # Same for each omini stream with weight == 0.
                if any_omini_off:
                    offset = omini_start
                    for omini_len, omini_weight in zip(omini_lens_bs, omini_weights_bs):
                        if omini_weight <= 0.0:
                            cc_mask[img_start:img_end, offset:offset + omini_len] = float('-inf')
                            cc_mask[offset:offset + omini_len, img_start:img_end] = float('-inf')
                        offset += omini_len

                # Block omini_i - omini_j (i != j)
                if len(omini_lens_bs) > 1:
                    offset_i = omini_start
                    for i, len_i in enumerate(omini_lens_bs):
                        offset_j = omini_start
                        for j, len_j in enumerate(omini_lens_bs):
                            if i != j:
                                cc_mask[offset_i:offset_i + len_i, offset_j:offset_j + len_j] = float('-inf')
                            offset_j += len_j
                        offset_i += len_i

                cross_control_masks.append(cc_mask)
        ...
        if cross_control_masks:
            cross_control_masks = torch.stack(cross_control_masks, dim=0).unsqueeze(1)
            if attention_mask_method != "adaptive":
                cross_control_masks = cross_control_masks.expand(-1, num_attention_heads, -1, -1)
        else:
            cross_control_masks = None

        return attention_masks, hard_attention_masks, cross_control_masks
    ...
    def __call__(
            ...
            max_sequence_length: int = 512,
            regional_max_sequence_length: int = 50,
            omini_conds: Optional[List[torch.Tensor]] = None,
            omini_adapter_names: Optional[List[str]] = None,
            omini_cond_scale_factor: int = 1,
            omini_cond_weights: Optional[List[float]] = None,
            omini_cond_position_deltas: Optional[List[Optional[Tuple[int, int]]]] = None,
    ):
        r"""
        ...
        use_omini = omini_conds is not None and len(omini_conds) > 0
        ...
        # 3b. Prepare OminiControl condition images (one per condition type, each with its own adapter)
        omini_cond_hidden_states_list = None  # list of tensors, one per condition type
        omini_cond_ids_list = None  # list of tensors, one per condition type
        omini_cond_seq_lens = None  # list[list[int]]: per-batch list of per-condition seq lengths

        if use_omini:
            if omini_adapter_names is None or len(omini_adapter_names) != len(omini_conds):
                raise ValueError("omini_adapter_names must have one entry per omini_conds condition")

            omini_cond_height = height // omini_cond_scale_factor
            omini_cond_width = width // omini_cond_scale_factor

            omini_cond_hidden_states_list = []
            omini_cond_ids_list = []
            per_cond_seq_lens = []  # seq len of each omini condition

            # Per-stream position delta
            if omini_cond_position_deltas is not None and \
                    len(omini_cond_position_deltas) != len(omini_conds):
                raise ValueError(
                    f"omini_cond_position_deltas has "
                    f"{len(omini_cond_position_deltas)} entries but "
                    f"omini_conds has {len(omini_conds)} — must match."
                )

            for omini_idx, omini_cond in enumerate(omini_conds):
                # Prepare and VAE-encode each omini condition
                omini_img = self.prepare_image(
                    image=omini_cond,
                    width=omini_cond_width,
                    height=omini_cond_height,
                    device=device,
                    dtype=self.vae.dtype,
                )
                assert len(omini_img) == batch_size

                omini_latent = self.vae.encode(omini_img.to(self.vae.dtype)).latent_dist.sample()
                omini_latent = (omini_latent - self.vae.config.shift_factor) * self.vae.config.scaling_factor
                omini_latent = self._pack_latents(
                    omini_latent,
                    batch_size=omini_latent.shape[0],
                    num_channels_latents=omini_latent.shape[1],
                    height=omini_latent.shape[2],
                    width=omini_latent.shape[3],
                )

                omini_id = self._prepare_latent_image_ids(
                    omini_img.shape[0],
                    omini_img.shape[-2] // self.vae_scale_factor // 2,
                    omini_img.shape[-1] // self.vae_scale_factor // 2,
                    device,
                    prompt_embeds.dtype,
                )
                # Apply scale bias for position encoding
                omini_scale_bias = (omini_cond_scale_factor - 1.0) / 2
                omini_id[..., 1:] = omini_id[..., 1:] * omini_cond_scale_factor + omini_scale_bias

                # Apply per-stream position delta if configured.
                if omini_cond_position_deltas is not None and \
                        omini_cond_position_deltas[omini_idx] is not None:
                    dh, dw = omini_cond_position_deltas[omini_idx]
                    omini_id[..., 1] += dh
                    omini_id[..., 2] += dw

                per_cond_seq_lens.append(omini_latent.shape[1])
                # Repeat for num_images_per_prompt
                omini_cond_hidden_states_list.append(omini_latent.repeat(num_images_per_prompt, 1, 1))
                omini_cond_ids_list.append(omini_id.repeat(num_images_per_prompt, 1, 1))

            # Build per-batch seq lens
            omini_cond_seq_lens = [per_cond_seq_lens for _ in range(batch_size)]
            omini_cond_seq_lens = [item for item in omini_cond_seq_lens for _ in range(num_images_per_prompt)]
        ...
        if use_regional_control:
            ...
            attention_mask, hard_attention_mask, cross_control_mask = self.prepare_attention_mask(
                ...
                omini_cond_seq_lens=omini_cond_seq_lens,
                omini_cond_weights=omini_cond_weights,
            )
            ...
            self._joint_attention_kwargs["cross_control_mask"] = cross_control_mask

            if do_true_cfg:
                neg_joint_attention_kwargs = {"cond2image_attention_weight": cond2image_attention_weight}
                neg_attention_mask, neg_hard_attention_mask, neg_cross_control_mask = self.prepare_attention_mask(
                    attention_mask_method=attention_mask_method,
                    regional_labels=regional_labels,
                ...
                neg_joint_attention_kwargs["cross_control_mask"] = neg_cross_control_mask
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Add OminiControl conditioning
                if use_omini:
                    transformer_kwargs["omini_cond_hidden_states_list"] = omini_cond_hidden_states_list
                    transformer_kwargs["omini_cond_ids_list"] = omini_cond_ids_list
                    transformer_kwargs["omini_adapter_names"] = omini_adapter_names
